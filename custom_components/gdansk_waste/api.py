...
def normalize_text(value: str) -> str:
    """Normalize street names and user input for comparisons."""
...
class ResolvedAddress:
    """Address resolved against the Gdansk schedule API."""
    ...
    def unique_id(self) -> str:
        return f"{self.town_id}:{self.street_id}:{normalize_house_number(self.house_number)}"

    @property
    def label(self) -> str:
        details = [detail for detail in (self.group_name, self.district, self.sides) if detail]
        if not details:
...
@dataclass(slots=True)
class ScheduleData:
    """Parsed schedule data for a resolved address."""
...
class GdanskWasteApiClient:
    """Client for Gdansk waste collection schedule endpoints."""
    ...
    async def async_get_current_period(self) -> dict[str, Any]:
        ...
        return max(periods, key=lambda period: str(period.get("startDate", "")))
    ...
    async def async_resolve_address_candidates(
        ...
        *,
        ...
        if groups:
            ...
            candidates = self._parse_resolved_addresses(
                initial_result,
                town_id=town_id,
                town_name=town_name,
                house_number=house_number,
    ...
    async def async_select_address(
        ...
        *,
        ...
        candidates = await self.async_resolve_address_candidates(
            street=street,
            house_number=house_number,
            preferred_group=preferred_group,
            preferred_street_id=preferred_street_id,
    ...
    async def async_fetch_schedule(self, address: ResolvedAddress) -> ScheduleData:
        ...
        for schedule_item in schedules:
            ...
            raw_days = str(schedule_item.get("days", ""))
            ...
            for raw_day in raw_days.split(";"):
                ...
                try:
                    ...
                    _LOGGER.debug("Skipping invalid date in schedule: %s", schedule_item)
                    continue
    ...
    def _merge_choosed_street_ids(self, streets: list[dict[str, Any]]) -> str:
        ...
        for street in streets:
            raw_ids = str(street.get("choosedStreetIds", ""))
            identifiers.extend(
        ...
        for street in details.get("streets", []):
            resolved.append(
                ResolvedAddress(
                    ...
                    group_description=str(
                        street.get("schedulegroup") or fallback_group_name
                    ),
    ...
    def _deduplicate_candidates(
        ...
        for candidate in candidates:
            key = f"{candidate.street_id}:{candidate.group_name}:{candidate.sides}"
    ...
    def _candidate_sort_key(
        ...
        *,
        ...
        number_score = 0
        if normalize_house_number(house_number) == normalize_house_number(candidate.house_number):
            number_score -= 1

        group_score = 0 if candidate.group_name else 1
        return (preferred_score, number_score, group_score, candidate.label)
